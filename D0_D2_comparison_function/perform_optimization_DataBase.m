...
for ktest=1:1:length(Tests)
    time_A = cputime;
    Chosen = Tests{ktest};
    [P_Var,D,t,Tau,d,tdet] = Reading_Data_Base(Chosen,DB_path,Df_S);
    ...
    p=plot(3.5*t./(tdet),error,'Color','r',LineWidth=0.4,LineStyle=ls);
    alpha(p,.1)
    grid on 
    box on
    xlim([0,10])
...
FIG4=figure(4);
    ...
    hold on 
    error2 = abs((((D_0D2DW)./(D))))-1;
    p=plot(3.5*t./(tdet),error2,'Color','k',LineWidth=0.4,LineStyle=ls);
    alpha(p,.1)
    grid on 
    box on
    xlim([0,10])
