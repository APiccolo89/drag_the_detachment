...
for ktest = 1:flength
    ...
    if ~isempty(D_0D)
        ...
        hold on
        plot(t,topo(1:length(t))./(abs(min(topo(1:length(t))))),"Color",cmap(index_color1,:),LineWidth=0.8,LineStyle="-")
        ax = gca;
        ...
        ax.XGrid = 'on';
        ax.YGrid = 'on';
        ax.XColor = [0 102 204 ]./255;
        ax.YColor = [0 102 204 ]./255;
        ax.LineWidth = 1.4;
        ax.XMinorTick = 'on';
        ax.YMinorTick = 'on';
        %ax.XTickLabel =[];
