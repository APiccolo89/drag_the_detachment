...
if nlm.islinear==0  
    ...
    % Slab Rheology
    eta0DS_v  = 10.^[23];                      % [Pas] reference diffusion creep viscosity of the slab 
    xiUS_v     = [1000.0];               % [n.d.]viscosity contrast between diffusion and dislocation creep at the reference stress 
    n_v       = [3.5];            % [n.d.] pre-exponential factor
    % Upper Mantle
    eta0DM_v  = 10.^[17,18,19,20,21];                       % [Pas] reference diffusion creep viscosity of the upper mantle 
...
z = Data_S.Psi; 
scatter(x,y,10,log10(t_c_factor),'filled');
colorbar 
grid on
set(gca, 'XScale', 'log')
