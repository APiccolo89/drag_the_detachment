...
if nlm.islinear==0
    ...
    if Psi == 0
        ...
        xiUM_v    = 10.^[-6,-2,-1,0,1,2,4];                  % [n.d.] viscosity contrast between diffusion and dislocation creep at reference stress (UM)
    else
        %Geometric properties of the Slab
        l0_v      = (300e3:50e3:600e3);         % initial length [m]
        D0_v      = 80e3;                      % thickness [m]
...
if nlm.islinear == 1
    ...
    filename_ = '../Data_Base/NonLinear_Tests_Data_Base_n3_iteration_high_T_initial_guess_EXP.mat';
    [nTv,~] = ndgrid(eta0DM_v,s0_v,eta0DS_v,l0_v,xiUS_v,xiUM_v,D0_v,n_v);
    number_tests = length(nTv(:));
    initial_vectors.number_tests = number_tests;
end
